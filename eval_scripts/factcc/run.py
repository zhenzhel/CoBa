...
from utils import (compute_metrics, convert_examples_to_features, output_modes, processors, simple_accuracy)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        logger.info("  Full dataset length = %d", eval_dataset.full_ds_length)
        ...
        # adjust acc by full ds length
        adjusted_acc = result['acc'] * len(eval_dataset) / eval_dataset.full_ds_length
        result['acc'] = adjusted_acc
           
        eval_loss = eval_loss / nb_eval_steps
        # result["loss"] = eval_loss
        results.update(result)

        output_base_file_name = os.path.basename(args.file_name).replace(".jsonl", "")
        output_eval_file = os.path.join(eval_output_dir, f"{output_base_file_name}_factcc_eval_results.txt")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    filename_=args.file_name.split('/')[-1][:-6]

    logger.info("Creating features from dataset file at %s", args.data_dir)
    label_list = processor.get_labels()
    # examples = processor.get_dev_examples(args.data_dir) if evaluate else processor.get_train_examples(args.data_dir)
    examples = processor.get_dev_examples(args.data_dir, filename_) if evaluate else processor.get_train_examples(args.data_dir)
    full_ds_length = len(examples)
    ...
    dataset.full_ds_length = full_ds_length
    return dataset
...
def main():
    ...
    parser.add_argument("--file_name", default=None, type=str, required=True,
                        help="the filename of jsonl file to eval")
